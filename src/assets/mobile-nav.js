(function(){
  ...
  ready(function(){
    var cleanRoutes = {
      ...
      "course-paid": "/course-paid/",
      "course-corporate": "/course-corporate/",
      "course-detail": "/course-detail/",
      "corporate": "/corporate/",
      "reviews": "/reviews/",
      "faq": "/faq/",
      "instructor": "/instructor/",
      "instructor-apply": "/instructor-apply/",
      "application-complete": "/application-complete/"
    };

    function cleanUrlFromLegacyHtml(url){
      if(!url || url.origin !== window.location.origin) return "";
      var match = url.pathname.match(/\/html\/([^/]+)\.html$/i);
      if(!match) return "";
      var key = match[1].toLowerCase();
      var route = cleanRoutes[key];
      if(!route) return "";
      return url.origin + route + url.search + url.hash;
    }

    var currentCleanUrl = cleanUrlFromLegacyHtml(window.location);
    if(currentCleanUrl){
      window.history.replaceState(window.history.state, document.title, currentCleanUrl);
    }
    ...
    document.addEventListener("click", function(event){
      ...
      var normalized = cleanUrlFromLegacyHtml(new URL(link.getAttribute("href"), document.baseURI));
